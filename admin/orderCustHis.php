...
if (isset($_GET['search_date'])) {
    ...
    $sql = "SELECT * FROM orders o, customer c, product_detail pd WHERE o.customerID = c.customerID AND o.ordersID = pd.ordersID AND adminID = '$id' AND DATE(o.orderDate) = '$search_date' GROUP BY o.ordersID ORDER BY o.ordersID DESC LIMIT $start, $limit";

    $result = mysqli_query($conn, $sql);
    $counter = 1;
} else {
    // Default query to fetch all orders with pagination
    $sql = "SELECT * FROM orders o, customer c, product_detail pd WHERE o.customerID = c.customerID AND o.ordersID = pd.ordersID AND adminID = '$id' GROUP BY o.ordersID ORDER BY o.ordersID DESC LIMIT $start, $limit";

    $result = mysqli_query($conn, $sql);
    $counter = 1;
}

$countQuery = "SELECT COUNT(*) AS total FROM orders o, customer c, product_detail pd WHERE o.customerID = c.customerID AND o.ordersID = pd.ordersID AND adminID = '$id'";
...
<body>
    ...
    <div class="container">
        <a href="orderCust.php" class="btn btn-dark">Back</a> <br><br>
            ...
            <tbody>
                ...
                ?>
                    <tr>
						...
						<th><center><?php echo $row['p_status'] ?></center></th>
                        <th><center><a href="custOrderDetail.php?id=<?php echo $row['ordersID'] ?>" class="link-dark" style="text-decoration: none;"><i class="fa fa-download"></i>&nbsp;Details</a></center></th>
                    </tr>
                <?php
                    $counter++;
                }
